...
<%include file='pyfr.solvers.eulerReacting.kernels.flux'/>
...
<%pyfr:macro name='rsolve' params='ul, ur, n, nf'>
    ...
    fpdtype_t al,ar;
    ${pyfr.expand('inviscid_flux', 'ul', 'fl', 'pl', 'vl', 'al')};
    ${pyfr.expand('inviscid_flux', 'ur', 'fr', 'pr', 'vr', 'ar')};
   ...
   // fpdtype_t a = sqrt(${c['gamma'] - 1}*(H - 0.5*nv*nv));
    ...
    // reformulate to calculate multicomponent

   // fpdtype_t sl = nv - a;
   // fpdtype_t sr = nv + a;
    fpdtype_t sl = min(nvl-al,nvr-ar);
    fpdtype_t sr = max(nvl+al,nvr+ar);     
...
% elif i >= (nvars-N):                                                    
    ...
    nf_frstar = s_star*(sr*ur[${i}] - nf_fr) * rcp_rstar;    
% else:
    d_star = ${'s_star' if i == nvars - N -1 else 'n[{0}]'.format(i - 1)};  
